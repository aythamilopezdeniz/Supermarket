...
//        articles.add(new Article("Leche Puleva Entera", "images/articulos/lacteos/leche/puleva-entera-escalar.jpg", "3.45", "alimentacion", "lacteos", "leche", "Leche entera con calcio PULEVA 1L."));
    ...
    public static ArrayList getTypeArticle(String type) {
        ...
        for (Article article : articles) {
            if(article.getTipo().equals(type) || article.getSubtipo1().equals(type) || article.getSubtipo2().equals(type))
    ...
    public static ArrayList searchArticle(String article) {
        ...
        for (Article product : articles) {
            if(product.getNombre().equals(article) || product.getTipo().equals(article) || 
                    product.getSubtipo1().equals(article) || product.getSubtipo2().equals(article))
    ...
    private static void establishedDiscount(NoDescuento descuento) {
        ...
        for (Article article : articles) {
            if(article.getTipo().equals(types.get(0)) || article.getSubtipo1().equals(types.get(0)) || article.getSubtipo2().equals(types.get(0)))
                updateAmount(article, Math.round((article.getPvp()-(article.getPvp()*(new Double(sales)/100.0)))*100.0)/100.0);
            if(article.getTipo().equals(types.get(1)) || article.getSubtipo1().equals(types.get(1)) || article.getSubtipo2().equals(types.get(1)))
                updateAmount(article, Math.round((article.getPvp()-(article.getPvp()*(new Double(sales)/100.0)))*100.0)/100.0);
            if(article.getTipo().equals(types.get(2)) || article.getSubtipo1().equals(types.get(2)) || article.getSubtipo2().equals(types.get(2)))
                updateAmount(article, Math.round((article.getPvp()-(article.getPvp()*(new Double(sales)/100.0)))*100.0)/100.0);
        }
    }

    private static void updateAmount(Article article, double price) {
